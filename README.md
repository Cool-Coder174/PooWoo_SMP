...
|---------|---------|-----------------|--------------------------------------------|
| Thunder | 3.4.1   | `tools/`        | Convert Java resource packs to Bedrock     |

To convert a Java resource pack for Bedrock players: `java -jar tools/Thunder.jar <input-pack.zip>`
...
PooWoo_SMP/
  ...
  backups/              Timestamped backups (generated by backup.bat)
...
| `max-players`              | 52            | Maximum concurrent players            |
| `difficulty`               | normal        | Default difficulty                    |
| `gamemode`                 | survival      | Default game mode                     |
| `online-mode`              | true          | Require legitimate Minecraft accounts |
| `enforce-secure-profile`   | false         | Must be false for Geyser/Floodgate    |
...
Bedrock Edition players (phone, console, Windows 10/11) can join the server thanks to Geyser. Floodgate lets them connect with just a Microsoft account — no Java account needed.

### Bedrock Connection Info

| Platform | Address          | Port  |
|----------|------------------|-------|
| Java     | (your IP):25565  | 25565 |
| Bedrock  | (your IP):19132  | 19132 |

Bedrock players will appear in-game with a `.` prefix (e.g. `.Steve`) by default.

### Known Limitations

Some things don't translate perfectly between Bedrock and Java — see the [current limitations](https://geysermc.org/wiki/geyser/current-limitations/). Key ones:

- No clickable links in chat
- No glowing effect
- Banner layers limited to 6
- Custom anvil/smithing recipes don't work
- Bamboo/dripstone collision issues (mitigated by Hurricane plugin)

### Hurricane Plugin

Hurricane fixes bamboo and dripstone collision for Bedrock players by removing their server-side collision. Its config is at `plugins/Hurricane/config.yml` (generated on first boot).

### Windows Loopback Fix

If you host the server and play Bedrock on the **same Windows PC**, you must run this once in an admin PowerShell:

```
CheckNetIsolation LoopbackExempt -a -n="Microsoft.MinecraftUWP_8wekyb3d8bbse"
```

This allows the Bedrock client to connect to `localhost`. Not needed for other devices on the network.
...
ipconfig
```
Look for `IPv4 Address` (e.g., `192.168.1.100`). They connect to `192.168.1.100:25565` (Java) or `192.168.1.100:19132` (Bedrock).
...
**How it works:**
```
Java    --> GCP VM (34.71.32.17:25565) --> SSH tunnel (TCP)   --> Your PC (localhost:25565)
Bedrock --> GCP VM (34.71.32.17:19132) --> rathole tunnel (UDP) --> Your PC (localhost:19132)
Voice   --> GCP VM (34.71.32.17:24454) --> rathole tunnel (UDP) --> Your PC (localhost:24454)
```
...
- Cost: $0 (free tier)
- Firewall: TCP 25565 (Java), UDP 19132 (Bedrock), UDP 24454 (Voice Chat), TCP 2333 (rathole control)
...
- **"Outdated client/server"**: Players must use Minecraft Java Edition **26.1.1** (or connect via ViaVersion from older clients).
...
Run the backup script to copy all world data and plugin configs (skipping plugin `.jar` files) to a timestamped folder:

```
.\backup.bat
```

For a fully consistent backup, either stop the server first or run `save-all` in the server console before backing up. Backups are saved to `backups/` (gitignored).

**Manual backup** — copy these folders:
- `world/`
- `world_nether/`
- `world_the_end/`
